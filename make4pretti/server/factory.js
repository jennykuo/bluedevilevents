...
Factory.define('registeredUser', RegisteredUsers, {
	...
	firstName: function() {return Fake.user({fields: ['name'],}).name; },

	lastName: function() {return Fake.user({fields: ['surname'],}).surname; },
...
Factory.define('myEvent', MyEvents, {
	...
	startTime: function() {return Math.floor((Math.random() * 7) + 10) +
	":" + Math.floor((Math.random() * 49) + 10); },

	//End time b/w 18:10 and 23:59
	endTime: function() {return Math.floor((Math.random() * 5) + 18) +
	":" + Math.floor((Math.random() * 49) + 10); },
	...
	//Find the first name,last name, email, and netID of the RegisteredUser with _id = CreatorID
	//first: function() {return RegisteredUsers.find({_id: creatorID}).firstName; },
